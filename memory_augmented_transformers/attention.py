...
"""Altering Flax Attention Functions with Memory."""
...
fromflax.linen.module import merge_param
...
class MemoryAugmentedSelfAttention(MultiHeadDotProductAttention):
  """Self-attention special case of multi-head dot-product attention."""

  @compact
  def __call__(self, inputs_q: Array, memory: Array, mask: Optional[Array] = None, # type: ignore
               deterministic: Optional[bool] = None):
    """Applies multi-head dot product self-attention on the input data.
    Projects the inputs into multi-headed query, key, and value vectors,
    applies dot-product attention and project the results to an output vector.
    ...
    Returns:
      output of shape `[batch_sizes..., length, features]`.
    """
    # Attention mechanism over fixed memory
    input_k = memory[0:,:,:]
    input_v = memory[1:,:,:]
    memory_embedding =

    # Usual self-attention
    self_attention_embedding = super().__call__(inputs_q, inputs_q, mask,
                            deterministic=deterministic)

    # combine both embeddings
    # TODO(peterdavidfagan): test different ways of combining
    outputs = self_attention_embedding + memory_embedding
